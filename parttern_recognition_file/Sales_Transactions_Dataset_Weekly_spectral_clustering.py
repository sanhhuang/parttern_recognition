...
def GetAdjacentMatrixByKNN(dis_matrix, k, sigma=1.0):
    ...
    for j in neighbours_id:  # xj is xi's neighbour
        adjacent_matrix[i][j] = np.exp(- dis_atrix[i][j] / 2 / sigma / sigma)
...
if __name__ == '__main__': vec_data = np.array(
    read_Sales_Transactions_Dataset_Weekly.ReadSalesTransactionsDatasetWeekly('Sales_Transactions_Dataset_Weekly.csv'))
